...
out_dir = '/home/turbo/share/sda1/data/origin/out1' #새로 생생된 이미지 저장할 폴더
...
if os.path.exists(cfg_fname):
    ...
    for i in  range(len(lines)):
        if lines[i].find('<mode>') != -1:
            ...
            o_dir = out_dir.split('/')
            t_dir=''
            for d in range(len(o_dir)-1):
                t_dir += (o_dir[d]+'/')
            if not os.path.isdir(t_dir): os.mkdir(t_dir)
            if not os.path.isdir(out_dir): os.mkdir(out_dir)
