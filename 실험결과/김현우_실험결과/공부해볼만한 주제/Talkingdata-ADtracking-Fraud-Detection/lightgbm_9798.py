...
def lgb_modelfit_nocv(dtrain, dvalid, predictors, target='target', feval=None, early_stopping_rounds=20, num_boost_round=3000, verbose_eval=10, categorical_features=None,metrics='auc'):
    lgb_params = {
        ...
        'nthread': 12, # should be equal to REAL cores:http://xgboost.readthedocs.io/en/latest/how_to/external_memory.html
        'verbose': 0
        #         'device': 'gpu',
#         'gpu_platform_id':1
...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv("/home/rjs/바탕화면/adtrack/data/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
    ...
    train_df['in_test_hh'] = (2 
                         - 1*train_df['hour'].isin(most_freq_hours_in_test_data) 
                         + 1*train_df['hour'].isin(least_freq_hours_in_test_data)).astype('int8')
    ...
    train_df = do_countuniq( train_df, ['ip', 'day', 'in_test_hh'], 'app', 'X9','uint16', show_max=False ); gc.collect()
    ...
    train_df = do_count( train_df, ['ip', 'device', 'hour'], 'A3', show_max=False ); gc.collect()
    train_df = do_count( train_df, ['ip', 'day', 'in_test_hh'], 'A4', show_max=False ); gc.collect()
...
#----------------------------------------------------------------------------------------------------------------
    ...
    if not debug:
        print("writing...")
        sub.to_csv('/home/rjs/바탕화면/adtrack/result/sub_it%d.csv'%(fileno),index=False,float_format='%.9f')
    del sub
    gc.collect()
    print("All done...")
...
if __name__ == '__main__':
    ...
    nchunk=184903891 # 【The more the better】
    val_size=2500000
    #frm=nrows-75000000
    frm=0
    debug=False
    ...
    DO(frm,to,7) # fileno start from 0